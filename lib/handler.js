...
const handleCommand = async (ctx) => {
    ...
    // 📌 STICKY NOTE (MEMO) FEATURE
    if (body.startsWith('#memo')) {
        const memoContent = body.slice(5).trim();
        if (memoContent) {
            await Hardware.sendMemo(memoContent);
            await sock.sendMessage(jid, { text: '( ¬‿¬) Sticky note pinned!' }, { quoted: msg });
            return true;
        }
    }
