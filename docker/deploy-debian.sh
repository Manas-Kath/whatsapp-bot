...
# Bunty v5.6 - Test Deployment Script for Debian Home Server (CasaOS / Docker)
echo "=================================================="
echo "🤖 Deploying Bunty Bot Test Container on Debian"
echo "=================================================="
...
if [ ! -f ".env" ]; then
    if [ -f "docker/env.example" ]; then
        echo "⚠️ .env file missing. Copying docker/env.example to .env..."
        cp docker/env.example .env
        echo "👉 Please edit $APP_DIR/.env with your GEMINI_API_KEY before starting!"
    fi
fi
